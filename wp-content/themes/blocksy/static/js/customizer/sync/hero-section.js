...
export const renderHeroSection = prefix => {
	...
	if (prefix === 'author') {
		let meta_elements = getOptionFor('page_meta_elements', prefix)

		if (!meta_elements.joined) {
			;[
				...document.querySelectorAll('.hero-section .ct-meta-joined')
			].map(el => el.remove())
		}

		if (!meta_elements.articles_count) {
			;[
				...document.querySelectorAll('.hero-section .ct-meta-articles')
			].map(el => el.remove())
		}

		if (!meta_elements.comments) {
			;[
				...document.querySelectorAll(
					'.hero-section .ct-meta-comments-left'
				)
			].map(el => el.remove())
		}

		if (!document.querySelector('.hero-section .entry-meta > *')) {
			document.querySelector('.hero-section .entry-meta').remove()
		}
	}
